...
透明 TUN 模式只会出现在侧栏最下方的可展开高级区域，绝不会自动启用。风险
说明、恢复条件、能力检测结果、重新检测和设置指南都放在这个区域中，不再提供
命令面板入口。打开指南前会显示模态风险警告，用户必须确认自己拥有服务器的
物理访问能力，或 BMC/IPMI/iDRAC/iLO 等带外管理能力。扩展不会创建 TUN
网卡、安装软件，也不会修改全局路由或 DNS。修改共享服务器的默认路由可能
导致 SSH 断线并影响其他用户，因此相关操作保持为手动流程，且应优先隔离在
network namespace 中。
