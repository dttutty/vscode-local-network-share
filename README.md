...
The tunnel exposes a `socks5h://` proxy. Proxy environment variables are not a
single universal standard, so compatibility depends on both the tool and its
version. The table below is based on the linked official documentation.

- ✅ Works directly with the current SOCKS5h tunnel in a new integrated terminal.
- ⚠️ The tool supports proxies, but needs extra configuration or SOCKS support from an optional/runtime component.
- ❌ The current terminal variables and SOCKS5h endpoint are not sufficient.
...
| [APT](https://manpages.debian.org/unstable/apt/apt-transport-http.1.en.html#Proxy_Configuration) | ⚠️ | APT natively supports `socks5h://`, but `sudo` commonly removes proxy variables. Test with explicit forwarding: `sudo env http_proxy="$http_proxy" https_proxy="$https_proxy" no_proxy="$no_proxy" apt update`. Persistent APT configuration must cover both `Acquire::http::Proxy` and `Acquire::https::Proxy`. |
| [npm](https://docs.npmjs.com/cli/v11/using-npm/config/#https-proxy) | ✅ | Current npm honors `HTTP_PROXY` and `HTTPS_PROXY`; its current [proxy agent](https://github.com/npm/agent#features) supports SOCKS4/5. Test: `npm ping`. Older npm releases may differ. |
...
| [pip](https://pip.pypa.io/en/stable/user_guide/#using-a-proxy-server) | ⚠️ | pip reads `http_proxy` and `https_proxy` and accepts a proxy URL, but SOCKS operation depends on the installed pip/Requests/PySocks stack. Test: `python -m pip index versions pip`. Use an HTTP CONNECT bridge if it reports missing SOCKS support. |
| [Conda](https://docs.conda.io/projects/conda/en/stable/user-guide/configuration/settings.html#proxy-servers-configure-conda-for-use-behind-a-proxy-server) | ⚠️ | Conda reads `HTTP_PROXY` and `HTTPS_PROXY`, but its documented `.condarc` proxy schemes are HTTP/HTTPS; SOCKS behavior depends on its Python networking stack. Test: `conda search python`. Use an HTTP CONNECT bridge if it rejects `socks5h://`. |
| [Docker build/run](https://docs.docker.com/engine/cli/proxy/) | ⚠️ | Pass variables into the build or container, for example `docker build --build-arg ALL_PROXY --build-arg HTTP_PROXY --build-arg HTTPS_PROXY .`. The downloader inside the image/container must support SOCKS. |
| [Docker pull](https://docs.docker.com/engine/daemon/proxy/) | ❌ | Pulls are performed by `dockerd`, not by the terminal process. A systemd Docker daemon does not inherit these variables and requires separate daemon proxy configuration; the documented daemon configuration expects an HTTP/HTTPS proxy, so an HTTP CONNECT bridge is normally required. |
| [GNU Wget](https://www.gnu.org/software/wget/manual/html_node/Proxies.html) | ❌ | GNU Wget documents HTTP/HTTPS/FTP proxy URLs, not SOCKS or `ALL_PROXY`. Use curl instead, or place an HTTP CONNECT bridge in front of the SOCKS tunnel. |
