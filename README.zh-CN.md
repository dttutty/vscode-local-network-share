...
- 桌面版 VS Code 中的 Microsoft **Remote - SSH**。不要求特定的 Remote-SSH
  版本；本扩展通过本地 OpenSSH 客户端建立反向转发，不依赖 Remote-SSH 的
  proposed API。如果没有安装 Remote-SSH，侧边栏会提供 **Open Remote - SSH**
  按钮。目前不支持 Dev Containers、WSL、Codespaces 和浏览器版 VS Code。
...
扩展通常会根据当前远程工作区 URI 自动推断 SSH 配置别名。请先在目标服务器上
打开一个文件夹；空的 Remote-SSH 窗口不会提供足够信息供扩展可靠识别目标，
因此侧栏会显示 **Remote folder required** 并禁用 Start，而不会让普通用户输入
陌生的 alias。如果已经打开远端工作区但仍无法推断，高级用户可以点击
**SSH target**，或直接设置 `localNetworkShare.sshTarget`，例如：
...
在 **APT and sudo** 覆盖项中展开 **Frequent commands**，即可看到每条完整命令。
命令显示在不可编辑的圆角代码卡片中，右上角提供小型复制图标。这样需要手动
处理的操作和原因会放在一起，同时用户仍能在粘贴到终端前检查完整内容。

**Proxy coverage**（包括其中的 APT 命令）只会在网络共享进入活动状态后显示；
停止共享时会再次隐藏，避免用户对尚未工作的代理端点执行命令。
