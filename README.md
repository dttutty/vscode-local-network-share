...
pip/PyPI, Conda channels, Hugging Face models and datasets, npm registries,
documentation sites, and other software sources.
...
- Microsoft **Remote - SSH** in a desktop VS Code Remote-SSH window. No specific Remote-SSH version is required; the extension uses the local OpenSSH client for reverse forwarding rather than a proposed Remote-SSH API. If Remote-SSH is unavailable, the sidebar provides an **Open Remote - SSH** button. Dev Containers, WSL, Codespaces, and browser-based VS Code are not supported yet.
...
| [Hugging Face Hub CLI (`hf`)](https://huggingface.co/docs/huggingface_hub/en/guides/cli) | ✅ | Current `huggingface_hub` uses `HTTP_PROXY` and `HTTPS_PROXY`; `hf` replaces the deprecated `huggingface-cli`. Test dataset access without downloading files: `hf download HuggingFaceH4/ultrachat_200k --repo-type dataset --dry-run`. Hub files may redirect through Xet/CDN hosts. For slow large downloads, try `HF_HUB_DOWNLOAD_TIMEOUT=30`; if Xet-specific transfers fail, `HF_HUB_DISABLE_XET=1` requests the regular HTTP fallback, although very large files may still require Xet. |
...
The main status view contains **TUN mode**, which switches directly to a custom page inside the Local Network Share Sidebar Webview instead of opening an editor tab or redundant modal. On every entry, only the physical/BMC-access warning and a centered **I acknowledge** button are shown; acknowledgement removes the warning and reveals a single control page. Server readiness is checked automatically, **Recheck** runs it again, and **Options** is collapsed by default. **Start** and **Stop** place the corresponding reviewable command into a remote terminal without pressing Enter; the user reviews it, executes it, and responds to sudo directly in that terminal. Start also enables Basic sharing when needed. Advanced TUN has no Command Palette entry and is never enabled automatically. Changing a shared server's default route can disconnect SSH and affect other users, so the page recommends an isolated network namespace and clearly marks global routing as high risk.
